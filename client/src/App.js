...
import TrademarkUI from './components/Trademark/index.js';
...
class App extends Component {
  ...
  componentDidMount = async () => {
    ...
    try {
      ...
      Trademark = require('../../contracts/Trademark.sol');
    } catch (e) {
      console.log(e);
    }
    try {
      ...
      if (!isProd) {
        ...
        if (Trademark.networks) {
          deployedNetwork = Trademark.networks[networkId.toString()];
          if (deployedNetwork) {
            instanceTrademark = new web3.eth.Contract(Trademark.abi, deployedNetwork && deployedNetwork.address);
          }
        }
        if (instance || instanceWallet || instanceTrademark) {
          ...
          this.setState(
            {
              ...
              trademark: instanceTrademark,
            },
            () => {
              this.refreshValues(instance, instanceWallet, instanceTrademark);
              setInterval(() => {
                this.refreshValues(instance, instanceWallet, instanceTrademark);
              }, 5000);
            },
  ...
  createMark = async (mark, id) => {
    const { accounts, trademark } = this.state;
    await trademark.methods.createMark(mark, id).send({ from: accounts[0] });
  ...
  getMark = async id => {
    ...
    // Get the value from the contract to prove it worked.
    const response = await trademark.methods.lookUptokenIdToMarkInfo(id).call();
    // Update state with the result.
          ...
          <div className={styles.contracts}>
            ...
            <div className={styles.widgets}>
              ...
              <TrademarkUI
                ...
                markResponse={this.state.markResponse}
                markResponseText={this.state.markResponseText}
                {...this.state}
              />
